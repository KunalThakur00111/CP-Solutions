...
void solve(){
    ...
    for(int i=0;i<q;i++){
        ...
        for(int j=2;j<=sz/2;j*=2){
            int block=(idx+j-1)/j;
            if(block%2==1){
                int k=(block-1)*j;
                k++;
                ll bf=(prefix[k+j-1]^prefix[k-1]^a[idx]^c);
                ll af=(prefix[k+2*j-1]^prefix[k+j-1]);
                if(af>bf) ans+=j;
            }
            else{
                int k=(block-1)*j;
                k++;
                ll af=(prefix[k+j-1]^prefix[k-1]^a[idx]^c);
                ll bf=(prefix[k-1]^prefix[k-j-1]);
                if(bf>=af) ans+=j;
            }
        }
        anss[i]=ans;
    }
    for(int i: anss) cout<<i<<endl;
}
int32_t main(){
    kunel
    //precompute();
