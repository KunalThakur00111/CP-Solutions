...
void solve(){
    ...
    for(int i=0;i<n+m;i++){
        if(i==0){
            cnt[i]=(c[i].second==0)?1:0;// 0 means from a, 1 from b
        }
        else{
            cnt[i]=cnt[i-1]+(c[i].second==0?1:0);
    ...
    for(int i=0;i<q;i++){
        ...
        ll a_wale=cnt[z-1];
        ll b_wale=z-cnt[z-1];
        if(a_wale<=x && b_wale<=y){
            ans[i]=sum;
        }
        else if(a_wale>x){// a1 a2 a3 a4 a5 a6 a7, b1 b2 b3 b4 b5 b6
            ll diff=a_wale-x;
            sum-=((a_wale==0)?0:a[a_wale-1]);
            sum+=((x==0)?0:a[x-1]);
            sum-=((b_wale==0)?0:b[b_wale-1]);
            sum+=((b_wale+diff==0)?0:b[b_wale-1+diff]);
            ans[i]=sum;
        }
        else if(b_wale>y){
            ll diff=b_wale-y;
            ...
            sum+=((y==0)?0:b[y-1]);
            sum-=((a_wale==0)?0:a[a_wale-1]);
            sum+=((a_wale+diff==0)?0:a[a_wale-1+diff]);
            ans[i]=sum;
        }
    }
