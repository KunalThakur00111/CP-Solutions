...
void solve(){
    ...
    for(int i=n-1;i>=0;i--){
        if(a[i]==1 && b[i]==1){
            if(!mp1.count(2) && !mp2.count(2)){
                ans+=(n-i);
                dp[i]=n-i;
            }
            else if((!mp1.count(2) && mp2.count(2))){
                ans+=(mp2[2]-i);
                dp[i]=mp2[2]-i;
            }
            else if((mp1.count(2) && !mp2.count(2))){
                ...
                int idx=min(mp1[2],mp2[2]);
                if(idx==mp1[2] && idx==mp2[2]){
                    int len=idx-i+dp[idx];
                    ans+=len;
                    dp[i]=len;
                }
                else{
            ...
            if(a[i]==b[i]){
                if(!mp1.count(a[i]+1) && !mp2.count(a[i]+1)){
                    dp[i]=n-i;
                }
                else if((!mp1.count(a[i]+1) && mp2.count(a[i]+1))){
                    dp[i]=mp2[a[i]+1]-i;
                }
                else if((mp1.count(a[i]+1) && !mp2.count(a[i]+1))){
                    dp[i]=mp1[a[i]+1]-i;
                    ...
                    int idx=min(mp1[a[i]+1],mp2[a[i]+1]);
                    if(idx==mp1[a[i]+1] && idx==mp2[a[i]+1]){
                        int len=idx-i+dp[idx];
                        dp[i]=len;
                    }
                    else{
            ...
            if(!mp1.count(1) && !mp2.count(1)){
                ans+=(n-i);
            }
            else if((!mp1.count(1) && mp2.count(1))){
                ans+=(mp2[1]-i);
            }
            else if((mp1.count(1) && !mp2.count(1))){
                ans+=(mp1[1]-i);
                ...
                int idx=min(mp1[1],mp2[1]);
                if(idx==mp1[1] && idx==mp2[1]){
                    int len=idx-i+dp[idx];
                    ans+=len;
                }
                else{
